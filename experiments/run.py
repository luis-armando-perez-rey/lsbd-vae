...
from lsbd_vae.metrics.dlsbd_metric import dlsbd, create_combinations_k_values_range
...
TORUS_DATASETS = {"arrow", "pixel4", "modelnet", "pixel8", "pixel16"}

tf.config.run_functions_eagerly(True)

# region ARGUMENTS SCRIPT
parser = argparse.ArgumentParser(description="Training of LSBDVAE model either using paths or with semi-supervised pairs")
parser.add_argument("--gpu", nargs="?", dest="gpu_num", type=str, default="0", help="gpu to be used")
parser.add_argument("--n_labels", nargs="+", dest='n_labels', type=int, default=[0],
                    help="List of number of labels used for training the model")
...
parser.add_argument("--npaths", nargs="?", dest="npaths", type=int, default=1000,
                    help="Number of paths")
parser.add_argument("--lpaths", nargs="?", dest="lpaths", type=int, default=2,
                    help="Length of paths")
parser.add_argument("--spaths", nargs="?", dest="spaths", type=int, default=1,
...
if args.dataset in TORUS_DATASETS:
    ...
    latent_parameters = {
        ...
        "latent_dims": [1, 1],
        "kl_weights": None,
        "kwargs_list": [{"log_t_limit": (-10, -5)}, {"log_t_limit": (-10, -5)}]
    }
else:
    latent_dim = 7
    latent_parameters = {
...
if args.dataset in TORUS_DATASETS:
    k_values = create_combinations_k_values_range(start_value=-10, end_value=10)
    lsbd_score, _ = dlsbd(latent_reps, k_values, be_verbose=True, factor_manifold="torus")
else:
    k_values = np.arange(-10, 11)
    lsbd_score, _ = dlsbd(latent_reps, k_values, be_verbose=True, factor_manifold="cylinder")
