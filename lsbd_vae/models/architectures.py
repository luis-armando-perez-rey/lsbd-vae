...
def encoder_decoder_dense(latent_dim: int, input_shape: Tuple = (28, 28, 1), activation: str = "relu",
    ...
    x_reconstruction = Reshape(input_shape)(h)
    decoder = Model(dec_in, x_reconstruction)
...
def encoder_decoder_vgglike_2d(latent_dim: int, input_shape: Tuple = (64, 64, 1), activation: str = "relu",
    ...
    n_conv_layers = len(filters_lst)  # number of convolutional layers
    # calculate sizes after convolutions (before the dense layers)
    conv_depth = filters_lst[-1]  # depth after convolutions
    conv_width = width // (pool_size ** n_conv_layers)  # width after convolutions
    conv_height = height // (pool_size ** n_conv_layers)  # height after convolutions
    ...
    h = x  # input to first convolutional layer
...
def encoder_decoder_dislib_2d(latent_dim: int, input_shape: Tuple = (64, 64, 1), activation: str = "relu",
    ...
    for i in reversed(range(1, n_conv_layers)):
        h = Conv2DTranspose(filters=filters_lst[i - 1], kernel_size=kernel_size, strides=strides, padding="same",
                            activation=activation)(h)
    x_reconstruction = Conv2DTranspose(filters=depth, kernel_size=kernel_size, strides=strides, padding="same",
                                       activation="sigmoid")(h)
    decoder = Model(dec_in, x_reconstruction)
